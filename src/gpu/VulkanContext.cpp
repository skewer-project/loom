...
// Derive (stage, access) mask pair for either the source or destination side
// of a layout transition. Centralising the mapping means every code path that
// transitions an image agrees on the synchronisation scope. Unknown layouts
// LOOM_ASSERT — adding support for a new layout requires extending this table,
// not bypassing it with zero masks.
namespace {
...
StageAccessPair sourceMasks(VkImageLayout layout) {
    switch (layout) {
        ...
        case VK_IMAGE_LAYOUT_PRESENT_SRC_KHR:
            // Present has finished; no writes outstanding from our perspective.
...
StageAccessPair destMasks(VkImageLayout layout) {
    switch (layout) {
        ...
        case VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL:
            return {VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT};
        case VK_IMAGE_LAYOUT_PRESENT_SRC_KHR:
            // Bottom-of-pipe: signal occurs after all work; the present engine
            // does the actual wait via the presentation semaphore.
