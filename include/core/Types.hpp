...
struct Node {
  ...
  protected:
    ...
    // Typed accessor for deep-pin inputs. Asserts the upstream payload is
    // `Kind::Deep` (mismatches caught at evaluation if they slip past
    // canAddLink) and returns the unwrapped `DeepRef`. Used by deep-image
